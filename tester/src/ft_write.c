...
void test_write(int fd, const void *data, size_t len, int id, ssize_t (*function)(int, const void *, size_t), int flag_write_or_ft) {
    ...
    if (ret < 0) {
        ...
        {
            printf("[❌] Test %d ft_write : error code = %d\n", id, errno);
            perror("ft_write");
        }
        else
        {
...
int all_ft_write(void) {
    ...
    int fd_regular = open("./tester/data/test_write.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd_regular < 0) {
        close(fd_ft);
        perror("open");
        return 1;
    ...
    test_both_write(fd_ft, fd_regular, "is not right length\n", 123154546, id++);
